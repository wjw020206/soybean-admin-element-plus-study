...
/**
 * 在应用挂载前渲染一个 loading 遮罩，避免白屏闪烁
 * - 读取本地存储的主题色和暗黑模式偏好，提前应用到 DOM
 * - 使用纯 HTML + CSS 构建加载动画，不依赖 Vue 实例
 */
export function setupLoading() {
  ...
  /** 将主题色拆解为 R、G、B 三个分量，用于 CSS 自定义属性 */
  const { r, g, b } = getRgb(themeColor);

  // 使用空格分隔的 RGB 值，方便配合 rgba() 动态调整透明度
  const primaryColor = `--primary-color: ${r} ${g} ${b}`;

  /** 将色板映射为一组 CSS 自定义属性，供 Logo SVG 的渐变色使用 */
  ...
  /** 如果用户上次选择了暗黑模式，提前给 html 加上 dark 类 */
  if (darkMode) {
  ...
  /**
   * 四个圆点分别定位在容器的四个角
   * 通过 animate-delay 错开脉冲动画，形成波浪式呼吸效果
   */
  ...
  /**
   * 组装 loading 遮罩的完整 HTML 结构
   * - Logo SVG：使用色板渐变色
   * - 旋转容器（animate-spin）包裹四个脉冲圆点
   * - 标题：从国际化中读取
   */
  const loading = `
...
/**
 * 返回 Logo 的 SVG 字符串
 * 渐变色使用 CSS 自定义属性（--logo-color-*），由外部动态注入
 */
function getLogoSvg() {
  const logoSvg = `<svg
        width="100%"
